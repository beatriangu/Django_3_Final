...
async def get_last_messages(room_name, numer=None):
    ...
    queryset = queryset.order_by('-timestamp')[:numer or 3].values('body', 'user__username', 'timestamp')
...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        # Send the last 3 messages to the new user
        messages = await get_last_messages(self.room_name)
        messages = messages[::-1]
        for message in messages:
            await self.send(text_data=json.dumps({'type': 'message', 'message': {
                'body': message['body'],
        ...
        if await add_user_in_room(self.username, self.room_name) == 1:
            await self.send_message_to_chatroom(
                f'{self.username} has joined the chat', username=self.username
            )
    ...
    async def disconnect(self, close_code):
        ...
        if await remove_user_from_room(self.username, self.room_name) == 0:
            await self.send_message_to_chatroom(
                f'{self.username} has left the chat'
            )
    ...
    # Receive user list from room group
